...
class PipelineRunner extends EventEmitter {
  ...
  run(options) {
    return new Promise((resolve, reject) => {
      const { harPath, outDir, noDesc = false, limit, fresh = false, speed, headless = true, userAgent, exportMode, exportFields, includeSellerData = true } = options;
      ...
      this.emit('log', { level: 'debug', message: `[pipelineRunner] Options : noDesc=${noDesc}  limit=${limit ?? '(aucun)'} | fresh=${fresh} | userAgent=${userAgent ? '(transmis)' : '(aléatoire)'} | cwd=${process.cwd()} | exportMode=${exportMode || 'default'} | exportFields=${Array.isArray(exportFields) ? `[${exportFields.length} champ(s)]` : '(toutes)'} | includeSellerData=${includeSellerData}` });
